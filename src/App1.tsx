...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (readingState === ReadingState.LISTENING && !isListening) {
            timer = setTimeout(() => {
                ...
                if (currentUser?.id === 100) addDebugLog('⚠️ 마이크 5초 이상 응답 없음 - 재시작');
                // abort로 확실히 초기화 후 재시작 (단순 start보다 안정적)
                abortListening();
                setTimeout(() => {
                    startListening();
                }, 500);
    ...
    }, [isListening, readingState, currentUser?.id, addDebugLog, startListening, abortListening]);
    ...
    // Effect to handle retrying a verse after STT has fully stopped (iOS only)
    // 안드로이드는 절 전환 시 abort/restart를 하지 않으므로 이 effect가 불필요
    useEffect(() => {
    ...
    }, [isRetryingVerse, isListening, startListening, currentUser?.id]);
    ...
    useEffect(() => {
        ...
        if (isMatch) {
            ...
            if (currentVerseIndexInSession >= sessionTargetVerses.length - 1) {
                ...
                // 다음 구절로 넘어가기 전 모든 인식 데이터 비우기 (잔상 방지)
                setTranscriptBuffer('');
                resetTranscript();
                setMatchedCharCount(0);
                ...
                // 구절 전환 시 마이크 처리
                if (isIOS) {
                    // iOS: abort/restart가 필요 (세션이 죽는 문제 대응)
                    setTimeout(() => {
                        if (currentUser?.id === 100) addDebugLog('🛑 iOS abortListening() 호출');
                        abortListening();
                        setIsRetryingVerse(true);
                    }, 50);
                }
                // Android: recognition은 continuous=true로 계속 실행됨
                // resetTranscript()가 이미 위에서 호출되어 버퍼가 초기화되었고,
                // ignoreResultsRef로 600ms간 잔여 결과를 무시함
                // → abort/restart 불필요 (race condition 제거)
            }
        }
