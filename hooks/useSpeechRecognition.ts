  ...
  const startListening = useCallback(() => {
    ...
    try {
      ...
      if (isIOS) {
        ...
        // iOS Safari에서 권한 관련 문제 해결을 위한 추가 처리
        // 마이크 액세스 권한 재확인을 위해 명시적인 권한 요청
        navigator.mediaDevices.getUserMedia({ audio: true })
          .then(() => {
            console.log('[useSpeechRecognition] iOS - Microphone permission confirmed');
            // 마이크 권한이 확인된 후 음성 인식 시작
            if (recognitionRef.current) {
              recognitionRef.current.start();
              setIsListening(true);
              setError(null);
          ...
          .catch((err) => {
            console.error('[useSpeechRecognition] iOS - Microphone permission error:', err);
            setError(`마이크 권한 오류: ${err.message}`);
            setIsListening(false);
          });
      } else {
        // 안드로이드 및 기타 플랫폼
